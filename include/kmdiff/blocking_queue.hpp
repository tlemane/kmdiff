...
namespace kmdiff {
  ...
  class BlockingQueue
  {
   public:
    ...
    {
      {
        std::unique_lock<spinlock> lock(m_queue_mutex);
   ...
   private:
    ...
    std::queue<T> m_queue;
    spinlock m_queue_mutex;
    std::condition_variable_any m_empty_condition;
    std::condition_variable_any m_full_condition;
